...
async def get_album(name_artist: str, name_album: str):
    ...
    lst_results = []
    # All albums if artist name is not supplied
    lst_results = await library.get_album(name_artist=name_artist, name_album=name_album)
    return lst_results
...
async def search_music(type: TagTypeSearch, part_string: str = ...):
    ...
    """
    lst_results = []
    lst_results = await library.search(type=type.value,
                                   filter=part_string)
    return lst_results
...
async def get_album_cover(name_album_artist: str, name_album: str):
    ...
    """
    dict_image = await library.get_album_cover(name_artist=name_album_artist, name_album=name_album)
