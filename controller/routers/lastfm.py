...
class PeriodTopPlayed(str, Enum):
    all_time = "overall"
    last_7_days = "7day"
    last_month = "1month"
...
async def check_user_credentials():
    ...
    if lastfm.check_user_data():
        ...
        raise HTTPException(
            status_code=401,
            detail="Let user (re-)authorize access to her/his Last.fm account",
        )


@router.get("/get-user-access/")
async def open_lastfm_permissions_page(
    username: str, background_tasks: BackgroundTasks
):
...
async def get_artist_bio(name_artist: str):
    ...
    if result is None:
        raise HTTPException(
            status_code=401,
            detail="App not authorized for last.fm or invalid user token",
        )
    return result
...
async def get_top_assets(
    ...
    limit: int = 1000,
    period: PeriodTopPlayed = PeriodTopPlayed.all_time,
):
    ...
    - **limit**: The number of assets you want to retrieve, max = 1000
    """
    lst_results = lastfm.get_top_assets(
        type_asset=type_asset.value, limit=limit, period=period.value
    )
    return lst_results
